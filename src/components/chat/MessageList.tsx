...
import { User, Bot, Loader2, FilePlus, FilePen, FileSearch, Check } from "lucide-react";
import { MarkdownRenderer } from "./MarkdownRenderer";

function getStrReplaceLabel(args: Record<string, unknown>) {
  ...
  switch (command) {
    ...
    case "view":
      return { icon: FileSearch, label: `Reading ${filename}` };
    default:
      return { icon: FilePen, label: filename || "Working on file" };
  }
}
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const isStrReplace = tool.toolName === "str_replace_editor";
                            const { icon: Icon, label } = isStrReplace
                              ? getStrReplaceLabel(tool.args as Record<string, unknown>)
                              : { icon: FilePen, label: tool.toolName };
